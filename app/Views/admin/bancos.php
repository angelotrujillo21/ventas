...
<body data-nadmin = "<?=$nAdmin?>">
                                        ...
                                        <div class="row my-2">
                                            <div class="col-12">
                                                <div class="d-flex align-items-center p-2">
                                                    ...
                                                    <div class="ml-auto">
                                                        <button id="btnCrearBanco" class="btn btn-gradient-primary btn-rounded btn-icon">
    ...
    <div class="modal fade" id="formCEBanco" tabindex="-1" role="dialog" aria-labelledby="formCELoteLabel" aria-hidden="true">
...
<script>
    $(function() {
        ...
        // Formulario Clientes
        $("#btnCrearBanco").on('click', function() {
            fncCleanAll();
            $("#formCEBanco").find(".modal-title").html('Nuevo Banco');
            $("#formCEBanco").data("nIdRegistro",0);
            $("#formCEBanco").modal("show");
        });

        // Submit del formulario de banco
        $("#formCEBanco").find("form").on('submit',function(event){
           
            event.preventDefault();
            
            var nIdRegistro      = $("#formCEBanco").data("nIdRegistro");
            ...
            fncGrabarBanco(formData,function(aryData){
                if(aryData.success){
                    fncCleanAll();
                    $("#formCEBanco").modal("hide");
    ...
    function fncEliminarBanco(nIdRegistro) {
        ...
        function(){
            ...
            fncEjecutarEliminarBanco( jsnData , function(aryData){
    ...
    function fncMostrarBanco(nIdRegistro , sOpcion ) {

        $( "#formCEBanco" ).data("nIdRegistro",nIdRegistro);
      
        var jsnData = {
            nIdRegistro: nIdRegistro
        };

        fncBuscarRegistroBanco(jsnData, function(aryResponse){
                ...
                if (aryResponse.success) {
                    ...
                    if(sOpcion == 'editar'){
                        fncEditForm("#formCEBanco" , "Editar Banco");
                    } else {
                        fncViewForm("#formCEBanco" , "Ver Banco");
                    }
    ...
    function fncCleanAll(){
        fncRemoveDisabled( $("#formCEBanco").find("form") );
        fncClearInputs( $("#formCEBanco").find("form") );
     }
    ...
    function fncGrabarBanco(formData, fncCallback) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: web_root + 'bancos/fncGrabarBanco',
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
    ...
    function fncBuscarRegistroBanco(jsnData, fncCallback) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: web_root +  'bancos/fncMostrarRegistro',
            data: jsnData ,
            beforeSend: function () {
                fncMostrarLoader();
            },
    ...
    function fncEjecutarEliminarBanco( jsnData , fncCallback ) {    
        $.ajax({
            type: 'post',
            url: web_root + 'bancos/fncEliminarRegistro',
            data: jsnData,
            dataType: 'json',
            beforeSend: function () {
                fncMostrarLoader();
