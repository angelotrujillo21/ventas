                            ...
                            <div class="row">
                               ...
                               <div class="col-12 col-md-12">
                                    <div class="form-group">
                                        <label for="aryIdProducto" class="col-form-label">Producto</label>
                                        <select class="form-control" name="aryIdProducto" id="aryIdProducto" multiple>
                                            <?php if (fncValidateArray($aryProductos)) : ?>
                                                <?php foreach ($aryProductos as $aryLoop) : ?>
                                                    <option value="<?= $aryLoop["nIdProducto"] ?>"><?= $aryLoop["sDescripcion"] ?></option>
                                                <?php endforeach ?>
                                            <?php endif ?>
                                        </select>
                                    </div>
...
<script>
    $(function() {
        ...
        $("#formFilter").find("form").on('submit', function(event) {
            ...
            var aryIdProducto    = $("#aryIdProducto").val().length > 0 ? $("#aryIdProducto :selected").map(function(nIndex, item) {return $(item).val();}).get() : $("#aryIdProducto option").map(function(nIndex, item) {return $(item).val();}).get();
            ...
            var jsnData = {
                aryIdProducto   : aryIdProducto,
                dFechaInicio    : $('#dFechaInicio').val(),
                dFechaFin       : $('#dFechaFin').val(),
            };

            fncPopulateReporteComisionProducto(jsnData, function(aryData) {
                bFilterTable  = true;
    ...
    function fncPopulateReporteComisionProducto(jsnData, fncCallback) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: web_root + 'pedidos/fncPopulateReporteComisionProducto',
            data: jsnData,
            beforeSend: function() {
                fncMostrarLoader();
            },
