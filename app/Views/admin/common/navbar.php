<div class="main-navbar sticky-top bg-white">
    <!-- Main Navbar -->
    <nav class="navbar align-items-stretch navbar-light flex-md-nowrap p-0">
        <?php if (isset($bShowMenu) && $bShowMenu === true) : ?>
            <div class="navbar__menu flex-row d-none d-md-block">
                <a id="btn-toogle-desktop" class="nav-link nav-link-icon text-center cursor-pointer color-plomo">
                    <i class="material-icons">reorder</i>
                </a>
            </div>
        <?php endif ?>

        <div class="main-navbar__search w-100 d-none d-md-flex d-lg-flex">
            <div class="input-group input-group-seamless ml-3">

                <div class="input-group-prepend">
                    <div class="input-group-text">
                        <i class="fas fa-chart-line" style="color: rgb(0, 123, 255);"></i>
                    </div>
                </div>
                <input id="titulo" class="navbar-search form-control text-uppercase" type="text" readonly style="background: white; color:black; font-size: 14px;" value="<?= isset($sTitulo) && !empty($sTitulo) ? $sTitulo : NOMBRE_SITIO ?>">

            </div>
        </div>
        <ul class="navbar-nav border-left flex-row ">


            <li id="li-noti" class="nav-item border-right dropdown notifications">
                <a class="nav-link nav-link-icon text-center" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="Notificaciones">
                    <div class="nav-link-icon__wrapper">
                        <i class="material-icons">&#xE7F4;</i>
                        <span id="count-noti" class="badge badge-pill badge-danger">0</span>
                    </div>
                </a>
                <div id="content-noti" class="dropdown-menu dropdown-menu-small" aria-labelledby="dropdownMenuLink" style="max-height: 100vh;overflow-y: scroll;height: auto;">

                </div>
            </li>



            <?php if ($user["nIdRol"] == 1) :   ?>
                <li class="nav-item border-right dropdown">
                    <a class="nav-link nav-link-icon text-center" href="<?= route('empresas') ?>" role="button" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Mis Empresas">
                        <div class="nav-link-icon__wrapper">
                            <i class="material-icons">business</i>
                        </div>
                    </a>
                </li>
            <?php endif ?>

            <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle text-nowrap px-3" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">
                    <img class="user-avatar rounded-circle mr-2" src="<?= !empty($user['sImagen']) && strlen($user['sImagen']) > 0 ? src('multi/' . $user['sImagen']) : src('app/avatar.jpg') ?>" alt="<?= $user['sNombre'] ?>">
                    <span class="d-none d-md-inline-block"><?= $user['sNombre'] ?></span>
                </a>
                <div class="dropdown-menu dropdown-menu-small">

                    <div class="dropdown-divider"></div>

                    <a class="dropdown-item" id="btnEditarUsuarioEmpleado" data-nid="<?= $user["nIdRol"] == 1 ? $user["nIdUsuario"] : $user["nIdEmpleado"] ?>" data-nrol="<?= $user["nIdRol"] ?>" href="javascript:;">
                        <i class="material-icons">manage_accounts</i> Editar
                    </a>

                    <a class="dropdown-item text-danger" href="<?= route('salir') ?>">
                        <i class="material-icons text-danger">&#xE879;</i> Salir
                    </a>

                </div>
            </li>

        </ul>
        <nav class="nav">
            <?php if (isset($bShowMenu)  && $bShowMenu == true) : ?>
                <a href="#" class="nav-link nav-link-icon toggle-sidebar d-md-inline d-lg-none text-center border-left menu-collap">
                    <i class="material-icons">&#xE5D2;</i>
                </a>
            <?php endif ?>
        </nav>
    </nav>
</div>