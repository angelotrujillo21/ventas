                                                ...
                                                <select name="nIdProducto" class="form-control" id="nIdProducto">
                                                    <?php if (fncValidateArray($aryProductos)) : ?>
                                                         <?php foreach ($aryProductos as $aryProducto) : ?>
                                                             <option data-nidubicacionalmacen="<?= $aryProducto["nIdUbicacionAlmacen"]?>" data-nequivalencia="<?= $aryProducto["nEquivalencia"]?>" data-simagen="<?= $aryProducto["sImagen"] ?>" data-nprecio="<?= nf($aryProducto["nPrecioCompra"]) ?>" value="<?= $aryProducto["nIdProducto"] ?>">
                                    ...
                                    <div class="col-12 col-md-6 mb-2">
                                        <div class="row no-gutters">
                                            <div class="col-12">
                                                <div class="d-flex align-items-center">
                                                    <div class="flex-grow-1 bd-highlight">
                                                        <p class="m-0 font-16">Ubicacion :</p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-12 my-2">
                                                <select name="nIdUbicacionAlmacen" class="form-control" id="nIdUbicacionAlmacen">
                                                    <?php if (fncValidateArray($aryUbicacionAlmacen)) : ?>
                                                         <?php foreach ($aryUbicacionAlmacen as $aryLoop) : ?>
                                                             <option value="<?= $aryLoop["nIdUbicacionAlmacen"] ?>">
                                                               <?= strup( $aryLoop["sNombre"] ) . " - " . strup( $aryLoop["sCodigo"] ) ?> 
                                                             </option>
                                                         <?php endforeach ?>
                                                    <?php endif ?>
                                                </select>
                                     ...
                                     <div class="table-responsive">
                                         <table id="table-detalle" class="table">
                                             ...
                                             <tfoot>
                                                 ...
                                                 <tr>
                                                     <td class="text-right" colspan="8">Igv</td>
...
<script>
    ...
    $(function() {
        ...
        $("#btnCrearMov").on('click', function() {
            ...
            $("#formMov").find(".modal-title").html('Nueva nota de ingreso');
        ...
        $("#form-add-producto").on('submit', function(event) {
            ...
            var nIdUbicacionAlmacen   =  $("#nIdUbicacionAlmacen").val() != null ? $("#nIdUbicacionAlmacen").find("option:selected").val().trim() : 0 ;
            var sUbicacionAlmacen     =  $("#nIdUbicacionAlmacen").val() != null ? $("#nIdUbicacionAlmacen").find("option:selected").text().trim() : "" ;
            ...
            var jsnRow = {
                ...
                sUbicacionAlmacen       : sUbicacionAlmacen, 
        ...
        $("#btnGuardarMov").on("click", function() {
            ...
            if(nTipoMovimiento == '0'){
                ...
                return false;
            } else if (aryDataDetalle.length == 0) {
                toastr.error('Error. Debe ingresar un producto por lo menos para generar la nota. Porfavor verifique');
                return false;
            } 
            ...
            var jsnData = {
                ...
                nEntradaSalida    : $("body").data("nentrada"),
            ...
            // console.log(jsnData);
            // return;
            fncGrabarMovimiento(jsnData, (aryResponse) => {
        ...
        $("#nIdProducto").on("change", function() {
            ...
            var nIdUbicacionAlmacen = $(this).find("option:selected").data("nidubicacionalmacen");
            
            $("#nIdUbicacionAlmacen").val(nIdUbicacionAlmacen).trigger("change");
            $("#nPrecio").val(nPrecio);
                ...
                if (aryDetalle.length > 0) {
                    ...
                    aryDetalle.forEach((aryItem, nIndex) => {
                        ...
                        var jsnRow = {
                            ...
                            nConvertir              : 0,
                            sUbica                  : aryItem.nEquivalencia,
                            nIdUbicacionAlmacen     : aryItem.nIdUbicacionAlmacen,
    ...
    window.fncGetDataTableCatalogo = function(sTable) {
        ...
        $(sTable).find("tbody").find("tr").each(function() {
            ...
            var nConvertir           = ($(this).find("td").eq(4).find(".equivalencia").length) > 0 ? $(this).find("td").eq(4).find(".equivalencia").val() : 0;
            var nIdUbicacionAlmacen  = $(this).find("td").eq(6).data("nidubicacionalmacen");
    ...
    window.fncDrawFilaProducto = function(jsnData) {
        ...
        sHtml = `<tr> 
                    ...
                    <td><div>${jsnData.nEquivalencia == 1 ? `<input autocomplete="off" value="${jsnData.nConvertir}" class="form-control font-12 equivalencia" />` : ``}</div></td>
                    <td><div>${jsnData.sProducto}</div></td>
                    <td data-nidubicacionalmacen="${jsnData.nIdUbicacionAlmacen}"><div>${jsnData.sUbicacionAlmacen}</div></td>
    ...
    function fncGrabarMovimiento(formData, fncCallback) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: web_root + 'movimientos/fncGrabarMovimiento',
            data: formData,
            beforeSend: function() {
                fncMostrarLoader();
            },
