...
<body data-nadmin = "<?=$nAdmin?>">
                                                ...
                                                <table data-toggle="table" id="tblPrincipal" data-url="<?= route('canjepuntos/fncPopulate') ?>" data-toggle="table" data-search="true" data-query-params="queryParams" toolbarAlign="left" data-show-refresh="true" data-pagination="true" data-toolbar="#toolbar" data-buttons-align="left" data-show-columns="true" data-pagination-h-align="left" data-pagination-detail-h-align="right" data-classes="table table-hover table-condensed" data-striped="true" data-buttons-class="gradient-primary-table" data-card-view="false" data-page-size="14" data-sort-name="" data-sort-order="asc">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th data-field="nPuntos" data-sortable="true">Valor Monetario</th>
    ...
    <div class="modal fade" id="formCECP" tabindex="-1" role="dialog" aria-labelledby="formCECPLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form>
                    <div class="modal-header">
                        <h5 class="modal-title" id="formCECPLabel">Nuevo Item</h5>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-md-6">
                                <div class="form-group">
                                     ...
                                     <input type="number" autocomplete="off" min="0.00" max="9999999.999999"  lang="en" step="0.000001" placeholder="" min="0.00" max="9999999.999999"  lang="en" step="0.000001"   class="form-control" name="nValorInicial" id="nValorInicial">
                                </div>
                            </div>
                            ...
                            <div class="col-12 col-md-6">
                                <div class="form-group">
                                     ...
                                     <input type="number" autocomplete="off" min="0.00" max="9999999.999999"  lang="en" step="0.000001" placeholder="" class="form-control" name="nValorFinal" id="nValorFinal">
                                </div>
                            </div>

                            <div class="col-12 col-md-6">
                                <div class="form-group">
                                     <label for="nPuntos" class="col-form-label">Valor Monetario</label>
                                     <input type="number" autocomplete="off" min="0.00" max="9999999.999999" lang="en" step="0.000001" placeholder="" class="form-control" name="nPuntos" id="nPuntos">
                                </div>
                            </div>
...
<script>
    $(function() {
        ...
        $("#btnCrearFAP").on('click', function() {
            fncCleanAll();
            $("#formCECP").find(".modal-title").html('Nuevo Item');
            $("#formCECP").data("nIdRegistro",0);
            $("#formCECP").modal("show");
        });

        // Submit del formulario 
        $("#formCECP").find("form").on('submit',function(event){
           
             event.preventDefault();

            var nIdRegistro        = $("#formCECP").data("nIdRegistro");
            var sDescripcion       = $("#sDescripcion").val();
            var nValorInicial      = $("#nValorInicial").val();
            var nValorFinal        = $("#nValorFinal").val();
            var nPuntos            = $("#nPuntos").val();
            ...
            if(nPuntos == '' || parseFloat(nPuntos) <= 0.00){
                toastr.error('Error. Ingrese el valor monetario para el canje de puntos. Porfavor verifique');
                return;
            } 
            ...
            if( parseFloat(nValorInicial) >= parseFloat(nValorFinal) ){
             ...
             fncGrabarCP(jsnData, function(aryData){
                 if(aryData.success){
                     fncCleanAll();
                     $("#formCECP").modal("hide");
    ...
    function fncEliminarCP(nIdRegistro) {
        ...
        function(){
            ...
            fncEjecutarEliminarCP( jsnData , function(aryData){
    ...
    function fncMostrarFAP(nIdRegistro , sOpcion ) {
        ...
        $( "#formCECP" ).data("nIdRegistro",nIdRegistro);
      
        var jsnData = {
            nIdRegistro: nIdRegistro
        };

        fncBuscarRegistroCP(jsnData, function(aryResponse){
                ...
                if (aryResponse.success) {
                    ...
                    $("#nPuntos").val(aryData.nPuntos);
                    $("#nEstado").val(aryData.nEstado);

                    if(sOpcion == 'editar'){
                        fncEditForm("#formCECP" , "Editar Item");
                    } else {
                        fncViewForm("#formCECP" , "Ver Item");
                    }
    ...
    function fncCleanAll(){
        fncRemoveDisabled( $("#formCECP").find("form") );
        fncClearInputs( $("#formCECP").find("form") );
     }
    ...
    function fncGrabarCP(jsnData, fncCallback) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: web_root + 'canjepuntos/fncGrabarCP',
            data: jsnData,
            beforeSend: function() {
                fncMostrarLoader();
            },
    ...
    function fncBuscarRegistroCP(jsnData, fncCallback) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: web_root +  'canjepuntos/fncMostrarRegistro',
            data: jsnData ,
            beforeSend: function () {
                fncMostrarLoader();
            },
    ...
    function fncEjecutarEliminarCP( jsnData , fncCallback ) {    
        $.ajax({
            type: 'post',
            url: web_root + 'canjepuntos/fncEliminarRegistro',
            data: jsnData,
            dataType: 'json',
            beforeSend: function () {
                fncMostrarLoader();
