...
<body data-nadmin = "<?=$nAdmin?>">
                                        ...
                                        <div class="row my-2">
                                            <div class="col-12">
                                                <div class="d-flex align-items-center p-2">
                                                    ...
                                                    <div class="ml-auto">
                                                        <button id="btnCrearUA" class="btn btn-gradient-primary btn-rounded btn-icon">
    ...
    <div class="modal fade" id="formCEUA" tabindex="-1" role="dialog" aria-labelledby="formCEUALabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form>
                    <div class="modal-header">
                        <h5 class="modal-title" id="formCEUALabel">Nuevo Ubicacion de almacen</h5>
...
<script>
    $(function() {
        ...
        // Formulario Clientes
        $("#btnCrearUA").on('click', function() {
            fncCleanAll();
            $("#formCEUA").find(".modal-title").html('Nueva Ubicacion de almacen');
            $("#formCEUA").data("nIdRegistro",0);
            $("#formCEUA").modal("show");
        });

        // Submit del formulario de Cliente
        $("#formCEUA").find("form").on('submit',function(event){
           
             event.preventDefault();

             var nIdRegistro        = $("#formCEUA").data("nIdRegistro");
             ...
             if (sNombre== '') {
                 toastr.error('Error. Ingrese un nombre para la ubicacion de almacen. Porfavor verifique');
                 return;
             } 
             ...
             fncGrabarUA(jsnData, function(aryData){
                 if(aryData.success){
                     fncCleanAll();
                     $("#formCEUA").modal("hide");
    ...
    function fncEliminarUA(nIdRegistro) {
        ...
        function(){
            ...
            fncEjecutarEliminarUA( jsnData , function(aryData){
    ...
    function fncMostrarUA(nIdRegistro , sOpcion ) {

        $( "#formCEUA" ).data("nIdRegistro",nIdRegistro);
      
        var jsnData = {
            nIdRegistro: nIdRegistro
        };

        fncBuscarRegistroUA(jsnData, function(aryResponse){
                ...
                if (aryResponse.success) {
                    ...
                    if(sOpcion == 'editar'){
                        fncEditForm("#formCEUA" , "Editar Ubicacion de almacen");
                    } else {
                        fncViewForm("#formCEUA" , "Ver Ubicacion de almacen");
                    }
    ...
    function fncCleanAll(){
        fncRemoveDisabled( $("#formCEUA").find("form") );
        fncClearInputs( $("#formCEUA").find("form") );
     }
    ...
    function fncGrabarUA(jsnData, fncCallback) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: web_root + 'ubicaciones/fncGrabarUA',
            data: jsnData,
            beforeSend: function() {
                fncMostrarLoader();
            },
    ...
    function fncBuscarRegistroUA(jsnData, fncCallback) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: web_root +  'ubicaciones/fncMostrarRegistro',
            data: jsnData ,
            beforeSend: function () {
                fncMostrarLoader();
            },
    ...
    function fncEjecutarEliminarUA( jsnData , fncCallback ) {    
        $.ajax({
            type: 'post',
            url: web_root + 'ubicaciones/fncEliminarRegistro',
            data: jsnData,
            dataType: 'json',
            beforeSend: function () {
                fncMostrarLoader();
