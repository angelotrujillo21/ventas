...
<body data-nadmin = "<?=$nAdmin?>">
                                        ...
                                        <div class="row my-2">
                                            <div class="col-12">
                                                <div class="d-flex align-items-center p-2">
                                                    ...
                                                    <div class="ml-auto">
                                                        <button id="btnCrearTipoCuenta" class="btn btn-gradient-primary btn-rounded btn-icon">
    ...
    <div class="modal fade" id="formCEBTC" tabindex="-1" role="dialog" aria-labelledby="formCELoteLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form>
                    <div class="modal-header">
                        <h5 class="modal-title" id="formCELoteLabel">Nuevo Tipo Cuenta</h5>
...
<script>
    $(function() {
        ...
        // Formulario Clientes
        $("#btnCrearTipoCuenta").on('click', function() {
            fncCleanAll();
            $("#formCEBTC").find(".modal-title").html('Nuevo Tipo de cuenta');
            $("#formCEBTC").data("nIdRegistro",0);
            $("#formCEBTC").modal("show");
        });

        // Submit del formulario de banco
        $("#formCEBTC").find("form").on('submit',function(event){
           
            event.preventDefault();
            
            var nIdRegistro      = $("#formCEBTC").data("nIdRegistro");
            var sNombre          = $("#sNombre").val().trim().toUpperCase();
            var sDescripcion     = $("#sDescripcion").val().trim().toUpperCase();
            var nEstado          = $("#nEstado").val();


            if(sNombre == ''){
                toastr.error('Error. Debe ingresar un nombre para el banco.Porfavor verifique');
                return false;
            }
            ...
            fncGrabarTipoCuenta(formData,function(aryData){
                if(aryData.success){
                    fncCleanAll();
                    $("#formCEBTC").modal("hide");
    ...
    function fncValidarRol (){
        if($("body").data("nadmin") == 1){
            ...
            $("#btnCrearTipoCuenta").hide();
        }
    }

    // Funciones de la tabla o layout Principal 

    function fncEliminarTC(nIdRegistro) {
        ...
        function(){
            ...
            fncEjecutarEliminarTC( jsnData , function(aryData){
    ...
    function fncMostrarTC(nIdRegistro , sOpcion ) {

        $( "#formCEBTC" ).data("nIdRegistro",nIdRegistro);
      
        var jsnData = {
            nIdRegistro: nIdRegistro
        };

        fncBuscarRegistroTC(jsnData, function(aryResponse){
                ...
                if (aryResponse.success) {
                    ...
                    if(sOpcion == 'editar'){
                        fncEditForm("#formCEBTC" , "Editar Tipo de cuenta");
                    } else {
                        fncViewForm("#formCEBTC" , "Ver Tipo de cuenta");
                    }
    ...
    function fncCleanAll(){
        fncRemoveDisabled( $("#formCEBTC").find("form") );
        fncClearInputs( $("#formCEBTC").find("form") );
     }
    ...
    function fncGrabarTipoCuenta(formData, fncCallback) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: web_root + 'tiposcuentas/fncGrabarTipoCuenta',
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
    ...
    function fncBuscarRegistroTC(jsnData, fncCallback) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: web_root +  'tiposcuentas/fncMostrarRegistro',
            data: jsnData ,
            beforeSend: function () {
                fncMostrarLoader();
            },
    ...
    function fncEjecutarEliminarTC( jsnData , fncCallback ) {    
        $.ajax({
            type: 'post',
            url: web_root + 'tiposcuentas/fncEliminarRegistro',
            data: jsnData,
            dataType: 'json',
            beforeSend: function () {
                fncMostrarLoader();
