            ...
            <div class="modal-content">
                <form>
                    <div class="modal-header">
                        <h5 class="modal-title" id="formCELoteLabel">Nuevo Extra</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-md-6">
                                <div class="form-group">
                                    <label for="nTipo" class="col-form-label">Seleccion</label>
                            ...
                            <div class="col-12 col-md-12">
                                <div class="form-group">
                                    <label for="sValores" class="col-form-label">Valores(Separados por comas)</label>
                                    <textarea autocomplete="off" placeholder="" class="form-control" name="sValores" id="sValores"></textarea>
                                </div>
                            </div>
...
<script>
    $(function() {
        ...
        $("#btnCrearRegistro").on('click', function() {
            fncCleanAll();
            $("#formCEBRegistro").find(".modal-title").html('Nueva Mesa');
        ...
        $("#formCEBRegistro").find("form").on('submit', function(event) {
            ...
            if (sNombre == '') {
                toastr.error('Error. Debe ingresar un nombre. Porfavor verifique');
                return false;
            }

            if (sValores == '') {
                toastr.error('Error. Debe ingresar los valores. Porfavor verifique');
                return false;
            }
            ...
            formData.append('nTipo', nTipo);
            formData.append('sValores', sValores);
    ...
    function fncMostrarRegistro(nIdRegistro, sOpcion) {
        ...
        fncBuscarRegistro(jsnData, function(aryResponse) {
            ...
            if (aryResponse.success) {
                ...
                $("#nTipo").val(aryData.nTipo);
                $("#sValores").val(aryData.sValores);
                $("#nEstado").val(aryData.nEstado);

                if (sOpcion == 'editar') {
                    fncEditForm("#formCEBRegistro", "Editar Extra");
                } else {
                    fncViewForm("#formCEBRegistro", "Ver Extra");
                }
    ...
    function fncGrabarRegistro(formData, fncCallback) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: web_root + 'cartaDigital/fncGrabarExtra',
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
    ...
    function fncBuscarRegistro(jsnData, fncCallback) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: web_root + 'cartaDigital/fncMostrarExtra',
            data: jsnData,
            beforeSend: function() {
                fncMostrarLoader();
            },
    ...
    function fncEjecutarEliminarRegistro(jsnData, fncCallback) {
        $.ajax({
            type: 'post',
            url: web_root + 'cartaDigital/fncEliminarExtra',
            data: jsnData,
            dataType: 'json',
            beforeSend: function() {
                fncMostrarLoader();
