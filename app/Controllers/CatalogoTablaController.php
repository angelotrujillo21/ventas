...
class CatalogoTablaController extends Controller
{
    ...
    public function fncPopulateConfig()
    {
        try {
            $sOpcion = isset($_POST['sOpcion']) ? $_POST['sOpcion'] : null;

            if (is_null($sOpcion)) {
                $this->exception("Error. No se han enviando todos los valores  .Porfavor verifique o solicite asistencia");
            }
            ...
            if ($aryResultado != false) {
                foreach ($aryResultado as $aryLoop) {
                    ...
                    $sActionShow      = "fncMostrarItemCatalogo(" . $aryLoop['nIdCatalogoTabla'] . ", 'ver' );";
                    $sActionEdit      = "fncMostrarItemCatalogo(" . $aryLoop['nIdCatalogoTabla'] . ", 'editar' );";
                    $sActionEliminar  = "fncEliminarItemCatalogo(" . $aryLoop['nIdCatalogoTabla'] . ");";
    ...
    public function fncGrabaCatalogoItem()
    {
        ...
        try {
            ...
            // Valida los valores del fomulario
            if ($nIdCatalogoTabla == null || $sNombreCatalogo == null  || $sDescripcionCortaItem == null || $sDescripcionLargaItem == null) {
    ...
    public function fncListado()
    {
        // Recibe valores del formulario
        $sCatalogo = isset($_POST['sCatalogo']) ? $_POST['sCatalogo'] : null;
        ...
        try {
            ...
            $aryData = $this->catalogoTablas->fncListado($sCatalogo);
