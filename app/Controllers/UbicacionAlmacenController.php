...
class UbicacionAlmacenController extends Controller
{
    //model principal
    public $users;
    public $ubicacionAlmacen; // Es mi modelo
    public $session;
    public $catalogotabla;
    public $ubigeo;
    public $sUrlUA = "ubicaciones-almacen";
    ...
    public function index()
    {
        try {
            ...
            $this->view('admin/ubicaciones-almacen', [
                'sTitulo'          => 'Mantenimientos de Ubicaciones de almacen',
                'user'             => $this->session->get('user'),
                'bShowMenu'        => true,
                "nAdmin"           => $this->fncIsAdmin($user["nIdRol"], $this->sUrlUA) ? 1 : 0,
            ]);
        } catch (Exception $ex) {
            echo $ex->getMessage();
        }
    ...
    public function fncPopulate()
    {
        try {
            ...
            $aryRows      = [];
            $aryData      = $this->ubicacionAlmacen->fncGetUA(["nIdEmpresa" => $user["nIdEmpresa"] , "nIdSede" => $user["nIdSede"]]);
            $bIsAdmin     = $this->fncIsAdmin($user["nIdRol"], $this->sUrlUA);

            if (is_array($aryData) && count($aryData) > 0) {
                foreach ($aryData as $aryLoop) {

                    $sActionShow      = "fncMostrarUA(" . $aryLoop['nIdUbicacionAlmacen'] . ", 'ver' );";
                    $sActionEdit      = "fncMostrarUA(" . $aryLoop['nIdUbicacionAlmacen'] . ", 'editar' );";
                    $sActionEliminar  = "fncEliminarUA(" . $aryLoop['nIdUbicacionAlmacen'] . ");";
    ...
    public function fncGrabarUA()
    {
        try {
            ...
            if ($nIdRegistro == 0) {
                ...
                $nIdNewEegistro = $this->ubicacionAlmacen->fncGrabarUA(
                    $user["nIdEmpresa"],
                    $user["nIdSede"],
                    $sNombre,
                    $sCodigo,
                ...
                //Actualizar
                $this->ubicacionAlmacen->fncActualizarUA(
                    $nIdRegistro,
                    $user["nIdEmpresa"],
                    $user["nIdSede"],
                    $sNombre,
            ...
            $sSuccess =  $nIdRegistro == 0 ? 'Ubicacion registrado exitosamente...' : 'Ubicacion actualizado exitosamente...';
    ...
    public function fncMostrarRegistro()
    {
        ...
        try {
            ...
            $aryData = $this->ubicacionAlmacen->fncGetUA(["nIdUbicacionAlmacen" => $nIdRegistro]);
    ...
    public function fncEliminarRegistro()
    {
        ...
        try {
            ...
            $this->ubicacionAlmacen->fncEliminarUA($nIdRegistro);
            $this->json(array("success" => 'Ubicacion eliminado exitosamente.'));
        } catch (Exception $ex) {
            echo $ex->getMessage();
        }
    }

    public function fncGetUA()
    {
        // Recibe valores del formulario
        ...
        try {
            ...
            $aryData = $this->ubicacionAlmacen->fncGetUA([
