...
function nf($number, $bIsTotal  = false)
{

    $nD = 2;

    if ($bIsTotal === false) {
        $decimal = fmod($number, 6); //0.25;

        $whole = (int) $number;  // -5
        $decimal  = $number - (int) $number;  // -.7
        ...
        if ($decimal !=  0) {
            $aryNumber = explode(".", $decimal);
            $nLenghtDecimal = strlen($aryNumber[1]);
        }

        // var_dump($decimal);
        // var_dump($aryNumber);
        // var_dump($nLenghtDecimal);

        if ($decimal == 0) {
            $nD = 2;
        } elseif ($nLenghtDecimal >=  1 && $nLenghtDecimal <= 2) {
            $nD = 2;
        } elseif ($nLenghtDecimal == 3) {
            $nD = 3;
        } elseif ($nLenghtDecimal == 4) {
            $nD = 4;
        } elseif ($nLenghtDecimal == 5) {
            $nD = 5;
        } else {
            $nD = 6;
        }
    }
    // var_dump($number, $decimal, $nLenghtDecimal , $nD);
    // echo "\n\n\n\n\n";
...
function fncGetFirstElement($array)
{
    return $array[0];
}
...
function fncCleanQuotes($str = null)
{
...
function fncEsMovil()
{
    // Verificar si el agente de usuario contiene alguna cadena de texto
    // comúnmente utilizada por dispositivos móviles
